...
import OrderStatusCard from "@/components/cards/OrderStatusCard";
import { Button } from "@/components/ui/button";
import StatusBlocksDashboard01 from "@/utils/dashboard/status-blocks";
import { useTranslation } from "react-i18next";
...
export default function HomeSec01() {
  ...
  const { CardData, StatusCardData } = StatusBlocksDashboard01();
  ...
  return (
    <div className="flex justify-between gap-6 lg:flex-row flex-col w-full">
      <section className="grid md:grid-cols-2  grid-cols-1 lg:w-[40%] w-full sm:gap-6 gap-3">
        ...
        {CardData.map((d, i) => (
          <Card
            ...
            icon={d.icon}
            label={t(d.label)}
            className={`${i === 0 ? "col-span-" : ""} shadow-[0px_8px_64px_0px_rgba(15,34,67,0.04)] dark:bg-inherit border bg-white `}
            bgcolor={d.bgcolor}
          />
        ))}
      </section>
      ...
      {/* Order Status================================================= */}
      <div className="lg:w-[60%] w-full ">
        <div className="bg-white dark:bg-inherit lg:p-5 sm:p-4 p-3  rounded-2xl shadow-[0px_8px_64px_0px_rgba(15,34,67,0.06)]">
          <div className="flex justify-between items-center  pb-10">
            <p className="text-xl capitalize font-semibold dark:text-neutral-300">Order Status</p>
            <Button variant="secondary">All orders</Button>
          </div>
          {/* Order Status================================================= */}
          <section className="grid md:grid-cols-3 sm:grid-cols-2 w-full sm:gap-6 gap-1">
            {StatusCardData.map((d, i) => (
              <OrderStatusCard
                key={i}
                id={d.id}
