...
// import StoreProductStockRequests from "../../table_actions/store/store-product-stock-requests";

type Props = {
  data: ProductLocalType[];
};

export default function DataTableStore({ data }: Props) {
  ...
  const columns: ColumnDef<ProductLocalType>[] = [
    ...
    {
      accessorKey: "store",
      header: () => <div className="font-bold text-black">Needed Stock</div>,
      cell: () => <div>
        S-12, M-32
        L-21, XL-89
      </div>,
    ...
    },
    {
      ...
      header: () => (
        <div className="font-bold text-black">Price / pieces</div>
      ),
      cell: () => <div className="flex flex-col">
       <span className="text-textGray text-xs"> 10-99 pieces</span>
        <span>$5.68</span>
      </div>,
    },
    {
    ...
    },
    {
      ...
      cell: () => <div>Store names</div>,
    },
    {
      accessorKey: "actions",
      header: () => <div className="font-bold text-black w-40 text-end "></div>,
      cell: ({ row }) => (
